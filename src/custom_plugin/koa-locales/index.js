...
module.exports = function (app, options) {
  ...
  for (let i = 0; i < localeDirs.length; i++) {
    ...
    for (let j = 0; j < names.length; j++) {
      const name = names[j]
      const filepath = path.join(dir, name, 'translate.json')
      // support en_US.js => en-US.js
      // const locale = formatLocale(name.split('.')[0]);
      const locale = formatLocale(name)
      let resource = {}
      ...
      // console.log('resources[locale]++',resources[locale]['api.requestParamsHasEspecialWord'])
    }
  }
