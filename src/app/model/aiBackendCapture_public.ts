 ...
 * @Last Modified time: 2020-07-16 16:04:27
 */
'use strict'
module.exports = app => {
  ...
  class Query {
    ...
    @Model
    async setRefPic (itemId, patrolObjId, patrolPointId, picUrl, modifier, cameraId = null, name = null) {
      const pk = UUID.v1()
    ...
    async savePoint (patrolItemId, patrolPointName, cameraId, deviceId, patrolObjId, methodId, sort, ptz, cameraName, orbitalId = null, orbitalPreset = null) {
      ...
      const pk = UUID.v1()
      const query = `
        INSERT INTO ` + scheme + `.TB_PATROL_POINT VALUES($pk,$patrolPointName,$sort,$patrolItemId,$patrolObjId,$methodId,null,$cameraId,$ptz,null,$orbitalPreset,'tb_camera',null,null,null,null,null,null,null,null,null,$nowDate,$nowDate,0,$deviceId,$orbitalId,0,$cameraName)
      `
      const res = await (this as any).query(
        query, {
          bind: {
            ...
            scheme,
            pk,
            patrolPointName: patrolPointName ? patrolPointName : '',
            sort,
            patrolItemId,
            patrolObjId,
            methodId,
            cameraId,
            ptz: ptz ? JSON.stringify(ptz) : null,
            nowDate,
            deviceId,
            cameraName
          }
    ...
    async savePointWithOrbital (patrolItemId, patrolPointName, cameraId, deviceId, patrolObjId, methodId, sort, ptz, orbitalId, preset, cameraName) {
      ...
      const pk = UUID.v1()
      const query = `
        INSERT INTO ` + scheme + `.TB_PATROL_POINT VALUES($pk,$patrolPointName,$sort,$patrolItemId,$patrolObjId,$methodId,null,$cameraId,$ptz,null,$preset,'tb_camera',null,null,null,null,null,null,null,null,null,$nowDate,$nowDate,0,$deviceId,$orbitalId,0,$cameraName)
      `
      const res = await (this as any).query(
        query, {
          bind: {
    ...
    @Model
    async getAIPointByCameraId (cameraId, methodId, patrolItemId, patrolObjId) {
      const query = `
          SELECT * FROM ` + scheme + `.TB_PATROL_POINT where camera_id = $cameraId and patrol_method_id = $methodId and patrol_item_id = $patrolItemId and patrol_obj_id = $patrolObjId and is_delete >=0
        `
      const res = await (this as any).query(
        query, {
          bind: {
